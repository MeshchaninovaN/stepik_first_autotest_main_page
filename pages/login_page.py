...
class LoginPage(BasePage):
    ...
    def register_new_user(self,email, password):
        email = self.browser.find_element(*LoginPageLocators.EMAIL_IN_FORM)
        email.send_keys(email)
        password = self.browser.find_element(*LoginPageLocators.PASSWORD_IN_FORM1)
        password.send_keys(password)
        password_repeat = self.browser.find_element(*LoginPageLocators.PASSWORD_IN_FORM2)
